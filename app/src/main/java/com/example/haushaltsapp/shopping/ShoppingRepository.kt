...
class ShoppingRepository {
    ...
    fun delete(id: String, ownerId: String) {
        deleteReference(id, ownerId)

        db.collection(SHOPPING_LISTS_COLLECTION)
            .document(id)
            .collection(UserRepository.MEMBERS_COLLECTION)
            .get()
            .continueWith { task ->
                if (task.isSuccessful) {
                    val members = task.result!!.toObjects(UserSummary::class.java)
                    members.forEach { user -> deleteReference(id, user.id) }
                }
            }
    ...
    fun deleteReference(id: String, userId: String) {
        db.collection(AuthRepository.USERS_COLLECTION)
            .document(userId)
            .collection(SHOPPING_LISTS_COLLECTION)
            .document(id)
            .delete()
    ...
    private fun deleteUsers(id: String) {
        db.collection(SHOPPING_LISTS_COLLECTION)
            .document(id)
            .collection(UserRepository.MEMBERS_COLLECTION)
            .delete(10, TAG)
